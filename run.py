...
st.markdown(
    ...
    "<p style='font-size:18px;'>Explorez, filtrez et estimez les spreads par secteur, échéance & rating</p>"
    "</div>", unsafe_allow_html=True
)
st.markdown("---")

# Préparation des filtres
essurde = df['secteur'].unique().tolist()
...
with tab1:
    ...
    sel_s = c1.multiselect("Secteurs", essurde, default=essurde)
    ...
    # Calcul des moyennes pour bubble et 3D
    grp = df_f.groupby(
        ['secteur','fourchette_annee','rating_num'], as_index=False
    )['spread'].mean()
...
with tab3:
    ...
    if not sub.empty:
        ...
        temp = df[
            (df['secteur']==ss) &
            (df['fourchette_annee']==se)
        ].assign(diff=abs(df['rating_num'] - alpha_to_num[sr]))
